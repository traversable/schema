...
import * as zx from '@traversable/zod'
import { z } from 'zod/v4'
import { Profunctor as Pro, symbol } from '@traversable/registry'
...
vi.describe('〖⛳️〗‹‹‹ ❲@traversable/zod❳', () => {
  ...
  vi.it.only('zx.getFallback', () => {
    vi.expect(zx.getFallback(Schema_03)).toMatchInlineSnapshot
      (`undefined`)

    vi.expect(zx.getFallback(Schema_03, symbol.coalesce)).toMatchInlineSnapshot
      (`undefined`)

    vi.expect(zx.getFallback(Schema_03, symbol.coalesce, symbol.union, 0)).toMatchInlineSnapshot
      (`
      {
        "one": {
          "tagB": "three",
    ...
    vi.expect(zx.getFallback(Schema_03, symbol.coalesce, symbol.union, 1)).toMatchInlineSnapshot
      (`
      {
        "tagA": "two",
        "two": {
    ...
    vi.expect(zx.getFallback(Schema_03, symbol.coalesce, symbol.union, 0, symbol.object, 'one')).toMatchInlineSnapshot
      (`
      {
        "one": undefined,
        "tagA": "one",
    ...
    vi.expect(zx.getFallback(Schema_03, symbol.coalesce, symbol.union, 0, symbol.object, 'one', symbol.union, 0)).toMatchInlineSnapshot
      (`
      {
        "one": {
          "tagB": "three",
    ...
    // vi.expect(zx.getFallback(Schema_01)).toMatchInlineSnapshot
    //   (`undefined`)

    // vi.expect(zx.getFallback(Schema_01, symbol.object, 'E')).toMatchInlineSnapshot
    //   (`undefined`)

    // vi.expect(zx.getFallback(Schema_01, symbol.object, 'E', symbol.coalesce, symbol.array)).toMatchInlineSnapshot
    //   (`
    //   [
    //     {
    //       "F": undefined,
  ...
  vi.it('zx.getSubSchema', () => {
    ...
    vi.expect(zx.getSubSchema(Schema_01)._zod.def.type).toMatchInlineSnapshot
      (`"object"`)
  ...
  vi.it('〖⛳️〗› ❲zx.getSchemaCursor❳', () => {
    ...
    // vi.expect(zx.getSchemaCursor(z.))
  })
