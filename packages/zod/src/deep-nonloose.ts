...
/** 
 * ## {@link deepNonLoose `deepNonLoose`}
 * 
 * Converts an arbitrary zod schema into its "deeply-non-loose" form.
 * 
 * That just means that the schema will be traversed, and all {@link z.looseObject `z.looseObject`}
...
export function deepNonLoose<T extends z.ZodType | z.core.$ZodType>(type: T): T
export function deepNonLoose<T extends z.ZodType | z.core.$ZodType>(type: T) {
  return F.fold<z.ZodType | z.core.$ZodType>((x, _i, original) => {
    ...
    switch (true) {
      ...
      case tagged('object')(x) && tagged('unknown')(x._zod.def.catchall): return z.object(clone._zod.def.shape)
    }
  })(type)
}

/** 
 * ## {@link deepNonLoose.writeable `zx.deepNonLoose.writeable`}
 * 
 * Convenience function that composes {@link deepNonLoose `zx.deepNonLoose`} 
 ...
 * {@link deepNonLoose.writeable `zx.deepNonLoose.writeable`}, you
...
deepNonLoose.writeable = fn.flow(deepNonLoose, toString)
