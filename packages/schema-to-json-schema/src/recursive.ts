...
export namespace Recursive {
  export const toJsonSchema: T.Algebra<t.Free, () => JsonSchema> = (x) => {
    switch (true) {
      ...
      case x.tag === URI.optional: return JsonSchema.OptionalSchema(x.def()).jsonSchema satisfies () => JsonSchema
      case x.tag === URI.eq: return JsonSchema.EqSchema(x.def).jsonSchema satisfies () => JsonSchema
      case x.tag === URI.array: return JsonSchema.ArraySchema(x.def()).jsonSchema
      case x.tag === URI.record: return JsonSchema.RecordSchema(x.def()).jsonSchema satisfies () => JsonSchema
      case x.tag === URI.union: return JsonSchema.UnionSchema(fn.map(x.def, (v) => v())).jsonSchema satisfies () => JsonSchema
      case x.tag === URI.intersect: return JsonSchema.IntersectSchema(fn.map(x.def, (v) => v())).jsonSchema satisfies () => JsonSchema
      case x.tag === URI.tuple: return JsonSchema.TupleSchema(fn.map(x.def, (v) => v())).jsonSchema satisfies () => JsonSchema
