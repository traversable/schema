...
import { Array_isArray, fn, has, Object_keys } from '@traversable/registry'
...
const is = {
  never: has('enum', (_): _ is [] => Array_isArray(_) && _.length === 0),
  null: has('type', (_) => _ === 'null'),
  boolean: has('type', (_) => _ === 'boolean'),
  integer: has('type', (_) => _ === 'integer'),
  number: has('type', (_) => _ === 'number'),
  string: has('type', (_) => _ === 'string'),
  union: has('anyOf', Array_isArray),
  intersection: has('allOf', Array_isArray),
  literal: has('const'),
  record: has('additionalProperties'),
  ...
  tuple: (x: any) => {
    return has('type', (_) => _ === 'array')(x)
      && has('prefixItems')(x)
      && has('minItems', (_) => typeof _ === 'number')(x)
      && has('items', (_) => _ === false)(x)
  },
  unknown: has(),
}// satisfies Record<string, (x: never) => boolean>
...
//       case has('enum', (_): _ is [] => Array_isArray(_) && _.length === 0)(x): return true
//     }
//   }
//   // !!x && typeof x === 'object' &&
//   // (
