...
function buildTupleCloner(x: JsonSchema.Tuple<Builder>, input: JsonSchema): Builder {
  if (!JsonSchema.isTuple(input)) return Invariant.IllegalState('cloneTuple', 'expected input to be a tuple schema', input)
...
function buildObjectCloner(x: JsonSchema.Object<Builder>, input: JsonSchema): Builder {
  if (!JsonSchema.isObject(input)) return Invariant.IllegalState('cloneObject', 'expected input to be an object schema', input)
...
function buildUnionCloner(x: JsonSchema.Union<Builder>, input: JsonSchema): Builder {
  if (!JsonSchema.isUnion(input)) return Invariant.IllegalState('cloneUnion', 'expected input to be a union schema', input)
  if (!areAllObjects(input.anyOf)) {
    return buildInclusiveUnionCloner(x.anyOf, input.anyOf)
  } else {
    const withTags = getTags(input.anyOf)
    return withTags === null
      ? buildInclusiveUnionCloner(x.anyOf, input.anyOf)
      : buildExclusiveUnionCloner(x.anyOf, withTags)
  }
}
...
export function deepClone(schema: JsonSchema) {
...
function deepClone_writeable(schema: JsonSchema, options?: deepClone.Options): string {
