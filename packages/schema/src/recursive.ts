...
const foldWithIndex
  : <T>(algebra: T.IndexedAlgebra<t.Functor.Index, t.Free, T>, initialIndex?: t.Functor.Index) => <S extends t.Schema>(term: S) => string
  = (algebra, initialIndex = t.defaultIndex) => (schema) => t.foldWithIndex(algebra)(schema, initialIndex) as never

/** TODO: parameterize `MAX_WIDTH` */
const MAX_WIDTH = 99
...
export function toString(schema: t.Schema, options?: toString.Options): string
export function toString(schema: t.Schema, options: toString.Options = toStringDefaults): string {
  const {
  ...
  return foldWithIndex<string>(
    (x, ix) => {
      ...
      switch (true) {
        /* v8 ignore next 1 */
        default: return fn.exhaustive(x)
        case x.tag === URI.eq: return `t.eq(${jsonToString(x.def, options, ix)})`
...
export function jsonToString(json: Json<any>, options?: toString.Options, ix?: t.Functor.Index): string
...
export function toTypeString(schema: t.Schema, options?: toString.Options, ix?: t.Functor.Index): string
export function toTypeString(schema: t.Schema, options: toString.Options = toStringDefaults, ix = t.defaultIndex) {
  const {
    initialOffset: OFF = toStringDefaults.initialOffset,
    format: FORMAT = toStringDefaults.format,
  } = options
