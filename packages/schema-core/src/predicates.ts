...
function isOptionalNotUndefinedSchema<T>(u: unknown): u is optional<T> {
...
export function union<T extends readonly ((u: unknown) => u is unknown)[]>(guard: readonly [...T]): (u: unknown) => u is T[number]
export function union<T extends readonly ((u: unknown) => u is unknown)[]>(qs: readonly [...T]) {
  return (u: unknown): u is never => qs.some((q) => q(u))
}

export function intersect<T extends readonly ((u: unknown) => u is unknown)[]>(guard: readonly [...T]): (u: unknown) => u is Intersect<T>
export function intersect<T extends readonly ((u: unknown) => u is unknown)[]>(qs: readonly [...T]) {
...
export function tuple$<Opts extends { minLength?: number } & SchemaOptions>(options: Opts):
  <T extends readonly Predicate[]>(qs: T)
    => (u: unknown)
