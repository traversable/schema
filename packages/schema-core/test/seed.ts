...
import { Equal, Predicate, t } from '@traversable/schema-core'
...
namespace Recursive {
  ...
  export const toSchema: T.Functor.Algebra<Seed.Free, t.LowerBound> = (x) => {
    if (x == null) return x
    switch (true) {
      default: return fn.exhaustive(x)
...
let entries = <T, U>(model: fc.Arbitrary<T>, constraints?: UniqueArrayDefaults<T, U>) => fc.uniqueArray(
  fc.tuple(
    fc.stringMatching(new RegExp('^[$_a-zA-Z][$_a-zA-Z0-9]*$', 'u')),
    model),
