...
let propertyValue = char().many({ not: alt(char(','), char('}')) }).map((xs) => xs.join(''))

let entry
  : <T>(parser: Parser<T>) => Parser<[k: string, v: T]>
  = (parser) => seq(
    key,
    optional(whitespace),
    char(':'),
...
let entriesWithOptionalDanglingComma
  : <T>(parser: Parser<T>) => Parser<[k: string, v: T][]>
  = (parser) => seq(
    index([entry(parser).trim(), char(',')], 0).many(),
    optional(entry(parser).trim()),
...
let entries
  ...
  = (parser) => index([
    char('{'),
    entriesWithOptionalDanglingComma(parser).trim(),
    char('}'),
  ], 1).map((_) => _ === null ? [] : _)

type JsonValue = null | boolean | string | number | { [x: string]: JsonValue } | JsonValue[];

interface Json {
  root: Parser<JsonValue>
  null: Parser<null>
  bool: Parser<boolean>
  string: Parser<string>
  number: Parser<number>
  array: Parser<JsonValue[]>
  object: Parser<Record<string, JsonValue>>
}

export let json = language<Json>({
  null: () => string('null').map(() => null),
  bool: () => alt(string('true'), string('false')).map((bool) => bool === 'true'),
  string: () => index([
    char('"'),
    char().many({ not: alt(char('"'), regexp(/\s/)) }),
    char('"'),
  ], 1).map((xs) => xs.join('')),
  number: () => digit.many().map((digits) => +digits.join('')),
  array: (
    (go) => index([
      char('['),
      go.root.many(),
  ...
  object: ((go) => {
    return index([
      char('{'),
      entriesWithOptionalDanglingComma(go.root).trim(),
      char('}'),
    ], 1).map((xs) => Object.fromEntries(xs))
  }),
  root: (go) => alt(...Object_values(go)),
})
