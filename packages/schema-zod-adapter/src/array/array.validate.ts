...
export function arrayValidate<S extends t.Schema>(
  itemsSchema: S,
  bounds?: { minLength?: number, maxLength?: number }
): ValidationFn<S['_type']>
export function arrayValidate<S>(
  itemsSchema: S,
  bounds?: { minLength?: number, maxLength?: number }
): ValidationFn<S['_type' & keyof S]>
//
export function arrayValidate(
  { def }: { def: unknown },
